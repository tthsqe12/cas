...
//std::cout << "dcode_sBlock: " << ex_tostring_full(e) << std::endl;
    ...
    for (ulong i = 0; i < n; i++)
    {
        ...
        if (ehas_head_sym_length(lli, gs.sym_sSet.get(), 2))
            rhs[i].setz(eval(ecopychild(lli,2)));
        else if (ehas_head_sym_length(lli, gs.sym_sSetDelayed.get(), 2))
            rhs[i].setz(ecopychild(lli,2));
    }
...
static ex _unique_symbol(er e)
{
    ...
    else if (eis_str(e))
    {
        ...
        const char * s = estr_data(e);
        size_t sn = estr_length(e);
...
//std::cout << "dcode_sUnique: " << ex_tostring_full(e) << std::endl;
    ...
    else
    {
        ...
        {
            size_t n = elength(f);
            uex g; g.init_push_backr(echild(f,0), n);
...
static ex _replace_vars(er e, er * a, uex * b, size_t n)
{
    if (eis_node(e))
    {
        ...
        uex f; f.init_push_backx(fh, len);
...
//std::cout << "dcode_sModule: " << ex_tostring_full(e) << std::endl;
    ...
    for (ulong i = 0; i < n; i++)
    {
        ...
        while (true)
        {
            ...
            lhs[i].reset(emake_sym(ecopy(esym_context(org[i])), emake_str(esym_name_mkstring(org[i]) + modulenumbers[j])));
        ...
        if (rhs[i].get() != nullptr)
            esym_ovalue(lhs[i].get()) = rhs[i].release();
    }
...
ex dcode_sWith(er e)
{
//std::cout << "dcode_sWith: " << ex_tostring_full(e) << std::endl;
    assert(ehas_head_sym(e, gs.sym_sWith.get()));
