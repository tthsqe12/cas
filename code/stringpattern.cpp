...
//std::cout << "pmake_prog(" << flags << "): " << ex_tostring_full(e) << std::endl;
    ...
    if (eis_str(e))
    {
        ...
        P.instructions.push_back(in);
        prev = in;
        for (size_t i = 0; i < estr_length(e); i++)
        {
            /* next */
            spprogram Q;
            spmake_prog_next(Q, flags);
            ...
            Q.instructions.clear();
            /* cmp */
            in = new spinstruction(spmnemonic_cmp, estr_data(e)[i]);
            P.instructions.push_back(in);
            prev->nexts.push_back(in);
            prev = in;
        }
...
//std::cout << "set_expr " << ex_tostring_full(e) << std::endl;
    ...
    cf->estring.clear();
    for (size_t i = 0; i < estr_length(e); i++)
        cf->estring.push_back(estr_data(e)[i]);
    ...
    if (!eis_node(e))
    {
        if (eis_sym(e))
        {
            for (size_t i = 0; i < symbols.size(); i++)
            {
                if (e == symbols[i].get())
                {
                    out.emplace_back(emake_str(vars[i].val));
                    return true;
                }
            }
        }
...
//std::cout << "dcode_sNames: " << ex_tostring_full(e) << std::endl;
    ...
    {
        M.clear_string();
        M.push_string(esym_context_mkstring(it->get()));
        M.push_string(esym_name_mkstring(it->get()));
        if (M.execute() != 0)
            v.emplace_back(ecopy(esym_name(it->get())));
    }

    for (auto it = gs.live_tsymbols.begin(); it != gs.live_tsymbols.end(); it++)
    {
        M.clear_string();
        M.push_string(esym_context_mkstring(*it));
        M.push_string(esym_name_mkstring(*it));
        if (M.execute() != 0)
            v.emplace_back(ecopy(esym_name(*it)));
    }
