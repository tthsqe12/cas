...
public:
    ...
    accumulator(int emptyset_, er head_, ex (*num_combine_)(er, er), ex (*expr_combine_)(ex)) :
    ...
    void push(ex T)
    {
        ...
        else
        {
            ...
            {
                estack.push_back(uex(
                    (*expr_combine)(emake_node(head.copy(), estack.size() - 1024, estack))
                ));
            }
        }
    }
...
static int _sum_nested(accumulator & A, er e, size_t pos)
{
    ...
    if (ehas_head_sym(it, gs.sym_sList.get()) && elength(it) >= 2 && elength(it) <= 4)
    {
        ...
        else if (elength(it) == 2 && eis_parray(lower.get()))
        {
            er l = eparray_get_normal(lower.get());
            size_t count = 0;
            while (++count <= elength(l))
            {
                if (value_type == 0)
                    esym_assign_ovaluex(sym.get(), ecopychild(l, count));
                else if (value_type == 1)
                    esym_assign_dvaluex(sym.get(), var.copy(), ecopychild(l, count));
                else if (value_type == 2)
                    esym_assign_svaluex(sym.get(), var.copy(), ecopychild(l,count));
...
//std::cout << "dcode_sSum: " << ex_tostring_full(e) << std::endl;
    ...
    accumulator A(0, gs.sym_sPlus.get(), &num_Plus, &ex_canonicalize_plus);
...
//std::cout << "dcode_sProduct: " << ex_tostring_full(e) << std::endl;
    ...
    accumulator A(1, gs.sym_sTimes.get(), &num_Times, &ex_canonicalize_times);
    int retcode = _sum_nested(A, e, 2);
    if (retcode == 0)
    {
        return A.release_sum();
