...
char* print_sym(char*s, er sym) {
  const char* p = esym_name_data(sym);
  while ((*s++ = *p++)) {}
  return --s;
}
...
printf("making code_list\n");
    ...
    ex code_list = nullptr/*emake_bin(4*(offset + 1))*/;
    u32 * p = nullptr/*(u32*)ebin_data(code_list)*/;
  for (std::vector<PNode>::iterator nit = prog.node.begin(); nit != prog.node.end(); ++nit) {
...
printf("making const_list  koffset=%d  roffset=%d\n",koffset,roffset);
    ex const_list = nullptr /*emake_bin(8*roffset)*/;
    unsigned char * sp = nullptr /* ebin_data(const_list)*/;
  memset(sp,0,roffset);
  for (std::map<Register,u32>::iterator it=prog.rmap.begin(); it!=prog.rmap.end(); ++it){
    if (it->first.var==0) {continue;}
...
std::cout << "E: " << ex_tostring_full(E) << std::endl;
    ...
    u32 * inst = nullptr /*(u32*)ebin_data(echild(E,1))*/;
    u32 * end = nullptr /*(u32*)(ebin_data(echild(E,1)) + ebin_size(echild(E,1)))*/;
    char * stack = nullptr /*(char*)ebin_data(echild(E,2))*/;
    u32 * ip = inst;
...
ex apply_compiledfunction(ex E)
{
    ...
    u32 * inst = nullptr /*(u32*)ebin_data(echild(E,0,1))*/;
    char * stack = nullptr /*(char*)ebin_data(echild(E,0,2))*/;
    u32 * ip = inst;
