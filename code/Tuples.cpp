...
//std::cout << "dcode_sTuples: " << ex_tostring_full(e) << std::endl;
    ...
    if (elength(e) == 2)
    {
        ...
        if (eis_int(p))
        {
            ...
            uex v; v.init_push_backr(gs.sym_sList.get(), fmpz_get_si(total_size.data));
            ...
            while (true)
            {
                uex l; l.init_push_backr(echild(f,0), n);
        ...
        else if (ehas_head_sym(p, gs.sym_sList.get()))
        {
            ...
            if (n == 0)
            {
                ...
                while (m != 0)
                {
                    ...
                    uex nans; nans.init_push_backr(echild(f,0), array_dims[m]);
            ...
            while (true)
            {
                ...
                for (size_t j = 0; j < r; j++)
                    l[j].init_push_backr(echild(f,0), array_dims[j]);
