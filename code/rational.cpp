    ...
    fmpz_t a, fmpz_t b, int sign)
{
    ...
    else
    {
        ...
        _fmpq_cfrac_list_push_back(s, M->_12);
...
ex dcode_iRealContinuedFraction(er e)
{
    ...
    {
        _fmpq_cfrac_list_push_back(s.data, eint_data(X));
    ...
    uex r(emake_parray_fmpq_cfrac_list(s.data));
    ex m = emake_list(
                emake_list(emake_int_move(M.data->_11), emake_int_move(M.data->_12)),
                emake_list(emake_int_move(M.data->_21), emake_int_move(M.data->_22)));
    return emake_list(r.release(), m);
}
...
//std::cout << "dcode_sContinuedFraction: " << ex_tostring_full(e) << std::endl;
    ...
    if (elength(e) == 1)
    {
        ...
        if (eis_real(X))
        {
            ...
            return emake_parray_fmpq_cfrac_list(s.data);
        }
        else if (eis_rat(X))
        {
            fmpz_set(x.data->left_num, fmpq_numref(erat_data(X)));
