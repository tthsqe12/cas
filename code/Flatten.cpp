...
{
//std::cout << "entering eval_flatten_sym h: " << ex_tostring_full(head) << "E: " << ex_tostring_full(E) << std::endl;
    assert(eis_sym(head));
    assert(ehas_head_sym(E, head));
...
//std::cout << "fn: " << fn << "  en: " << en << std::endl;
    ...
    uex e(E);
    uex p; p.init_push_backr(head, fn);
