...
{
    int32_t xdivs = 8, ydivs = 8;
    uex vertices; vertices.init_push_backr(gs.sym_sList.get(), 10+(xdivs+1)*(ydivs+1));
    uex triangles; triangles.init_push_backr(gs.sym_sList.get(), 10+2*xdivs*ydivs);
...
ContourPlot3D[z/2 - Sin[1 + x^2 + y^2]/(1 + x^2 + y^2), {x, -4, 4}, {y, -4, 4}, {z, -2, 2}]
...
ContourPlot3D[Cos[x + a y] + Cos[x - a y] + Cos[y + a z] + Cos[y - a z] + Cos[z + a x] + Cos[z - a x] - 2, {x,-5.2,5.2},{y,-5.2,5.2},{z,-5.2,5.2}]
...
*/
    uex r; r.init_push_backr(gs.sym_sGraphics3D.get(), 10);
/*
    r.push_back(cf.copy());
...
std::cout << "dcode_ContourPlot3D returning: " << ex_tostring(r.get()) << std::endl;
