...
//std::cout << "dcode_sNestList: " << ex_tostring_full(e) << std::endl;
    ...
    if (eis_int(e3) && fmpz_fits_si(eint_data(e3))
					&& (n = fmpz_get_si(eint_data(e3))) >= 0)
    {
        uex r; r.init_push_backr(gs.sym_sList.get(), n + 1);
	...
	if (elength(e) == 3)
	{
		...
		if (eis_node(l))
		{
	        ...
	        for (size_t i = 1; i <= elength(l); i++)
	            r.setz(eval(emake_node(ecopychild(e,1), r.release(), ecopychild(l,i))));
	        return r.release();			
		}
...
//std::cout << "dcode_sFoldList: " << ex_tostring_full(e) << std::endl;
	...
	if (elength(e) == 3)
	{
		...
		if (eis_node(l))
		{
	        uex r; r.init_push_backr(echild(l,0), elength(l) + 1);
			r.push_back(ecopychild(e,2));
	        for (size_t i = 1; i <= elength(l); i++)
	...
	else if (elength(e) == 2)
	{
		...
		if (eis_node(l))
		{
	        uex r; r.init_push_backr(echild(l,0), elength(l));
			if (elength(l) > 0)
			{
