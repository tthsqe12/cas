...
//std::cout << "dcode_sRealDigits: " << ex_tostring_full(e) << std::endl;
    ...
    if (elength(e) == 1)
    {
        ...
        if (eis_int(x))
        {
			...
			slong bound = 1 + fmpz_sizeinbase(eint_data(x), 10);
            fmpz * d = eto_parray_fmpz(r.get()).fit_edit(bound);
			...
			eto_parray_fmpz(r.get()).dimensions.set_index(0, n);
            flint_free(s);
        ...
        else if (eis_rat(x))
        {
            ...
            uex answer; answer.init_push_backr(gs.sym_sList.get(), 2 + fmpz_sizeinbase(u.data, 10));
