...
#define check(name) if (nat & ATTR_##name){l.push_back(wex(gs.sym_s##name.copy()));}
#define xcheck(name) if (xat & XATTR_##name){l.push_back(wex(gs.sym_s##name.copy()));}
        ...
        if (nat & ATTR_HoldAll)
        {
            er f = ((xat & ATTR_HoldAll) == ATTR_HoldFirst) ? gs.sym_sHoldFirst.get()
                  ...
                  : gs.sym_sHoldAll.get();
            l.push_back(wex(ecopy(f)));
        }
        check(HoldAllComplete)
        check(Listable)
//        check(Locked)
        if (nat & ATTR_NHoldAll)
        {
