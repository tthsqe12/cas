...
//std::cout << "rank: " << eparray_rank(e) << std::endl;
        ...
        if (epacked_type(e) != ETYPE_PACKED_FMPZ)
            return false;

        if (eto_parray_fmpz(e).dimensions.rank != 1)
            return false;

        slong n = eto_parray_fmpz(e).dimensions.get_index(0);
        fmpz * data = eto_parray_fmpz(e).array->data;

        p.resize(n + 1);
        p[0] = 0;
        for (slong i = 0; i < n; i++)
        {
            if ((ulong)data[i] > (ulong)n)
                return false;
            p[i + 1] = data[i];
        }
        return true;
...
*/
    ...
    uex s(emake_parray_fmpz());
    eto_parray_fmpz(s.get()).dimensions.set_rank1(S.size() - 1);
    fmpz * data = eto_parray_fmpz(s.get()).fit_edit(S.size() - 1);
    for (size_t i = 1; i < S.size(); i++)
    {
        data[i - 1] = S[i];
    ...
    uex o(emake_parray_fmpz());
    eto_parray_fmpz(o.get()).dimensions.set_rank1(O.size() - 1);
    data = eto_parray_fmpz(o.get()).fit_edit(O.size() - 1);
    for (size_t i = 1; i < O.size(); i++)
    {
        data[i - 1] = O[i];
        assert(data[i - 1] <= COEFF_MAX);
...
ex dcode_msGenerators(er e)
{
    ...
    uex r; r.init_push_backr(gs.sym_sList.get(), v.size());
    for (size_t i = 0; i < v.size(); i++)
