...
//std::cout << "dcode_sSameQ: " << ex_tostring_full(e) << std::endl;
    ...
    for (ulong i = 0; i + 1 < elength(e); i++)
    {
        if (!ex_same(echild(e,i+1), echild(e,i+2)))
            return gs.sym_sFalse.copy();
    }
...
{
//std::cout << "dcode_sEqual: " << ex_tostring_full(e) << std::endl;
    ...
    if (elength(e) == 2)
    {
        ...
        {
            ex z = num_Equal(echild(e,1), echild(e,2));
            return z == nullptr ? ecopy(e) : z;
        }

        if (ex_same(echild(e,1), echild(e,2)))
            return gs.sym_sTrue.copy();
    }
