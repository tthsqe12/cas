...
ex etake_list(er e, ulong i)
{
    ulong n = elength(e);
    if (i > n)
        return gs.const_empty_list.copy();

    uex r; r.init_push_backr(gs.sym_sList.get(), n - i);
    while (i <= n)
    {
        r.push_back(ecopychild(e,i));
        i++;
    }
    return r.release();
}
...
//std::cout << "boxbase_convert_from_ex: " << ex_tostring_full(e) << std::endl;
    if (eis_leaf(e))
    {
        if (eis_str(e))
        {
            ...
            size_t n = estr_length(e);
            const char * a = estr_data(e);
            for (size_t i = 0; i < n;)
            {
                char16_t c;
    ...
    if (ehas_head_sym(e, gs.sym_sRowBox.get()))
    {
        ...
        for (size_t i = 1; i <= elength(f); i++)
        {
            ...
            {
                size_t n = estr_length(echild(f,i));
                const char * a = estr_data(echild(f,i));
                for (size_t j = 0; j < n;)
                {
                    char16_t c;
            ...
            else
            {
                ...
                if (child->get_type() == BNTYPE_ROW)
                {
                    ...
                    us->child.push_back(new rowbegin);
    ...
    else if (ehas_head_sym(e, gs.sym_sMonoBox.get()))
    {
        ...
        for (size_t i = 1; i <= elength(f); i++)
        {
            ...
            if (eis_str(echild(f,i)))
            {
                const char * sp = estr_data(echild(f,i));
                size_t sn = estr_length(echild(f,i));
    ...
    else if (ehas_head_sym_length(e, gs.sym_sTagBox.get(), 2))
    {
        rowbox* inside = rowbox_from_ex(echild(e,1));
        return new tagbox(inside, ecopychild(e,2));
    }
...
*/
    ...
    else if (ehas_head_sym_length(e, gs.sym_sUnderscriptBox.get(), 2))
    {
        ...
        return new underscriptbox(body, under, 0);
    ...
    else if (ehas_head_sym(e, gs.sym_sCell.get()))
    {
        if (elength(e) < 1)
            return new errorbox(e);
        cellbox * us = new cellbox(rowbox_from_ex(echild(e,1)));
        for (ulong i = elength(e); i > 1; i--)
            us->options.push_ex_rule(echild(e,i));
        return us;
    }
    else if (ehas_head_sym(e, gs.sym_sCellGroupData.get()))
    {
        ...
        for (size_t i = 1; i <= elength(f); i++)
        {
            ...
            if (!(c->get_type() == BNTYPE_CELL || c->get_type() == BNTYPE_CELLGROUP))
            {
                ...
                r->child[1].cibox = iboxptr_make(new nullbox());
                c = new cellbox(r);
                dynamic_cast<cellbox*>(c)->options.push_pair(opt_style_string, gs.const_str_Input.copy());
            }
            us->child[i-1].cbox = c;
        }
    ...
    else if (ehas_head_sym(e, gs.sym_sNotebook.get()))
    {
        ...
        rootbox* us = new rootbox(nullptr, elength(f), 0, 0);
        for (size_t i = 1; i <= elength(f); i++)
        {
            boxbase* c = boxbase_from_ex(echild(f,i));
            if (!(c->get_type() == BNTYPE_CELL || c->get_type() == BNTYPE_CELLGROUP))
