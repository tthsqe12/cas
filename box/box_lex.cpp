...
void blexer::add_box(int btype)
{
    ...
    if (btype == BNTYPE_SUB ||
        btype == BNTYPE_SUPER ||
        btype == BNTYPE_SUBSUPER ||
        btype == BNTYPE_ROWBEGIN ||
        btype == BNTYPE_ROWEND)
    {
        p = 500;
    }
...
//if (c == CHAR_DifferentialD) {printf("got CHAR_DifferentialD\n");}
    ...
    case lexstate_oper_divide:
        ...
        if (c == ':') {
            ...
            type[idx - 1] = lextype_opinf_1st + 256*op_inf_RightComposition;
            state = lexstate_oper_divide_divide;
        } else {
            goto _lexstate_start;
        }
    ...
    case lexstate_oper_at:
        ...
        if (c == '@') {
            ...
            type[idx - 1] = lextype_opinf_1st + 256*op_inf_Composition;
            state = lexstate_start;
        } else {
            goto _lexstate_start;
        }
    ...
    case lexstate_num_after_exp:
        if (('0' <= c && c <= '9') || (c == '-') || (c == '+')) {
            p = 1000;
            t = lextype_number;
            state = lexstate_num_after_exp_num;
        } else {
            goto _lexstate_start;
        }
        break;

    case lexstate_num_after_exp_num:
        if (('0' <= c && c <= '9')) {
            p = 1000;
            t = lextype_number;
        } else {
...
void rowbegin::set_scolor(bsymer* T)
{
    return;
}

void rowend::set_scolor(bsymer* T)
{
    return;
}
...
void tagbox::set_scolor(bsymer* T)
{
...
size_t bsymer::color_subbox(size_t startj)
{
    ...
    boxbase * us_ = ibox_to_ptr(array[tokens[startj].idx].cibox);
    if (us_->get_type() == BNTYPE_UNDEROVER)
    {
        iboxarrayelem * sumvar = nullptr;
        Precedence prec = prec_none;
        underoverscriptbox* us = dynamic_cast<underoverscriptbox*>(us_);
        if (us->body.cbox->child.size() == 2)
        {
            if (ibox_is_char(us->body.cbox->child[0].cibox, CHAR_Sum))
                prec = prec_pre_Sum;
            else if (ibox_is_char(us->body.cbox->child[0].cibox, CHAR_Product))
                prec = prec_pre_Product;
        }

        if (prec != prec_none)
        {
            this->color_rowbox(us->over.cbox);

            rowbox* child = us->under.cbox;
            ...
            child->flags |= BNFLAG_COLORED;
            B.add_scolor(child->child.data(), 0, child->child.size() - 1);

            if (B.tokens[0].lextype == lextype_symbol_1st)
            {
                int32_t k = tokens[0].idx;
                sumvar = B.array + B.tokens[0].idx;
                iboxarrayelem* c = sumvar;
                do {
                    c->cibox = iboxchar_addsymtype(c->cibox, semtype_table);
                    c++;
            ...
            if (sumvar != nullptr)
            {
                vars_table.push_back(std::vector<iboxarrayelem*>(1,sumvar));
                startj = color_expr_inf_prec(startj + 1, prec);
                vars_table.pop_back();            
            }
            else
            {
    ...
    else if (us_->get_type() == BNTYPE_UNDER)
    {
        iboxarrayelem * sumvar = nullptr;
        Precedence prec = prec_none;
        underscriptbox* us = dynamic_cast<underscriptbox*>(us_);
        if (us->body.cbox->child.size() == 2)
        {
            if (ibox_is_char(us->body.cbox->child[0].cibox, CHAR_Sum))
                prec = prec_pre_Sum;
