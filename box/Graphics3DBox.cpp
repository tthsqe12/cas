...
void graphics3dbox::print(size_t indent, double offx, double offy)
{
    for (size_t i = 0; i < indent; i++)
        printf("    ");

    printf("(%f,%f) graphics3d: size(%f,%f:%f)\n",
...
void graphics3dbox::measure(boxmeasurearg ma)
{
    ...
    sizex = image.sizex + 2*5;
    sizey = image.sizey + 2*5;
    centery = sizey/2;
}
...
//    double T = 1 + x*x + y*y;
//    g->x = -2*x*cos(T)/T + (2*x*sin(T))/(T*T);
//    g->y = -2*y*cos(T)/T + (2*y*sin(T))/(T*T);
//    g->z = 0.5;
...
if ((j%10) == 0) std::cout << "row " << j << std::endl;
...
//std::cout << "N: " << N.x << ", " << N.y << ", " << N.z << std::endl;
        ...
        if (false)
        {
            ...
            {
                for (double theta = 0; theta < 2; theta += 1.0/(2+2*phi))
                {
...
//std::cout << "D: " << D.x << ", " << D.y << ", " << D.z << std::endl;
        ...
        double ss = t_depth*t_depth*(dir->x*dir->x + dir->y*dir->y + dir->z*dir->z)/(s*s);
        ss = (ss+100)/(4*ss+100);
        ...
        if (flipped)
        {
            vec3<double> ambcol0(0.05,0.20,0.90);
            vec3<double> ambcol1(0.05,0.35,0.85);
            vec3<double> ambcol2(0.00,0.25,0.90);
            vec3<double> ambcol3(0.10,0.20,0.85);
            ...
            fcolor.smul(mul/(w0+w1+w2+w3));
        }
        else
        {
            vec3<double> ambcol0(0.85,0.35,0.10);
            vec3<double> ambcol1(0.80,0.20,0.15);
            vec3<double> ambcol2(0.85,0.35,0.15);
            vec3<double> ambcol3(0.80,0.20,0.10);
            double w0 = std::max(0.001, vec3_dot(ambdir0,N));
            double w1 = std::max(0.001, vec3_dot(ambdir1,N));
            double w2 = std::max(0.001, vec3_dot(ambdir2,N));
            double w3 = std::max(0.001, vec3_dot(ambdir3,N));
        ...
        for (size_t k = 0; k < lights.size(); k++)
        {
            ...
            fcolor.madd(mul*ao*pow(std::max(0.0, vec3_dot(R,V)),24), light_spec[k]);
        }

        image.set_pixel(fcolor.x, fcolor.y, fcolor.z, t_depth, i, j);
    }
...
void graphics3dbox::draw_main(boxdrawarg da)
{
    ...
    for (int32_t x = 0; x < image.sizex; x++)
    {
        ...
        int32_t i = (5 + da.globx + x);
        int32_t j = (5 + da.globy + y);
