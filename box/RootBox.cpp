...
void rootbox::makecell(const char * s)
{
    if (cursor_b.empty())
    {
        // change our cell
        ...
        u->flags &= ~BNFLAG_MEASURED;
        u->options.remove(opt_style_string);
        u->options.push_pair(opt_style_string, emake_str(s));
        u->visit(visitarg_InvalidateAll);
        u->measure_quick(&nb->default_style);
        _fix_cell_insert();
        cursor_t.clear();
    ...
    {
        // make new cell
        rowbox * newbody = new rowbox(s);
        cellbox* newcell = new cellbox(newbody);
        newcell->options.push_pair(opt_style_string, emake_str(s));
...
{
    if (c->get_type() == BNTYPE_CELL)
        dynamic_cast<cellbox*>(c)->measure_quick(&nb->default_style);
...
//std::cout << "ok_parent_child" << std::endl;
//std::cout << "p: " << std::endl; p->print(0,0,0);
//std::cout << "c: " << std::endl; c->print(0,0,0);

    assert(dynamic_cast<cellbox*>(p)->flags & BNFLAG_MEASURED);
    assert(dynamic_cast<cellbox*>(c)->flags & BNFLAG_MEASURED);


    cellGroupType pstyle = dynamic_cast<cellbox*>(p)->get_cellgroupingtype();
    cellGroupType cstyle = dynamic_cast<cellbox*>(c)->get_cellgroupingtype();

    switch (pstyle) {
        case cellgt_Input:
            return cstyle == cellgt_Output || cstyle == cellgt_Message || cstyle == cellgt_Print;
        case cellgt_Message:
        case cellgt_Print:
        ...
        case cellgt_Text:
            return false;
        case cellgt_Subsubsection:
        case cellgt_Subsection:
        case cellgt_Section:
        case cellgt_Title:
            return cstyle < pstyle;
        default:
...
//std::cout << "ok_parent_child_splice" << std::endl;
//std::cout << "p: " << std::endl; p->print(0,0,0);
//std::cout << "c: " << std::endl; c->print(0,0,0);
...
//std::cout << "ok_parent_child_master" << std::endl;
//std::cout << "p: " << std::endl; p->print(0,0,0);
//std::cout << "c: " << std::endl; c->print(0,0,0);
//std::cout << "m: " << std::endl; m->print(0,0,0);

    assert(dynamic_cast<cellbox*>(p)->flags & BNFLAG_MEASURED);
    assert(dynamic_cast<cellbox*>(c)->flags & BNFLAG_MEASURED);
    assert(dynamic_cast<cellbox*>(m)->flags & BNFLAG_MEASURED);

    cellGroupType pstyle = dynamic_cast<cellbox*>(p)->get_cellgroupingtype();
    cellGroupType cstyle = dynamic_cast<cellbox*>(c)->get_cellgroupingtype();
    cellGroupType mstyle = dynamic_cast<cellbox*>(m)->get_cellgroupingtype();
...
void rootbox::print(size_t indent, double offx, double offy)
{
    for (size_t i = 0; i < indent; i++)
        printf("    ");
    printf("(%f,%f) root: size (%f,%f:%f) ", offx, offy, sizex, sizey, centery);
...
void rootbox::key_paste(boxbase*&b)
{
    ...
    else if (b != nullptr && b->get_type() == BNTYPE_ROW)
    {
        ...
        {
            dynamic_cast<cellbox*>(_us())->flags |= BNFLAG_CURSORAMARK;
            cell_insert(new cellbox(r, opt_style_string, gs.const_str_Input.copy()));
            cursor_t.clear();
            bool found = _find_mark(this, cursor_t, BNFLAG_CURSORAMARK);
            assert(found);
            dynamic_cast<cellbox*>(_us())->flags &= ~BNFLAG_CURSORAMARK;
...
void rootbox::insert_char(int32_t c)
{
    ...
    cellbox* newcell = new cellbox(newrow, opt_style_string, gs.const_str_Input.copy());
    cursor_t = cursor_a;
    cell_insert(newcell);
    cursor_a = cursor_t;
    cursor_b.clear();
...
insertRet rootbox::insert(boxbase*&b, insertArg m)
{
    ...
    switch (m)
    {
        ...
        case insertarg_Graphics3D:
        {
            ...
            cell_insert(new cellbox(newrow, opt_style_string, gs.const_str_Input.copy()));
            cursor_a = cursor_t;
            cursor_b.clear();
            return _us()->insert(b, m);
        }
...
ex eset_rule(er E, ex L, ex R)
{
    ex ru = emake_node(gs.sym_sRule.copy(), L, R);
    for (ulong i = 1; i < elength(E); i++)
    {
        er ei = echild(E,i);
        if (!ehas_head_sym_length(ei, gs.sym_sRule.get(), 2))
            continue;
        if (!ex_same(echild(ei,1), etor(L)))
            continue;
        return ereplacechild(E, 1, ru);
    }
    return eappend(ecopy(E), ru);
}

void rootbox::key_shiftenter()
{
    ...
    e.setz(emake_node(gs.sym_sEnterTextPacket.copy(), e.release()));
    swrite_ex(fp_in, e.get());
    fflush(fp_in);

    /* put line number on our cell */
    U->flags &= ~BNFLAG_MEASURED;
    U->options.remove(opt_CellLabel);
    ...
    if (nb->in_name.get() != nullptr)
        U->options.push_pair(opt_CellLabel, nb->in_name.copy());
    U->measure_quick(&nb->default_style);
    ...
    while (cursor_t[0] < child.size() &&
           ...
           dynamic_cast<cellbox*>(_us())->get_cellautooverwrite())
    {
        delete cell_remove();
    }
...
void rootbox::get_cursor(aftransform * T)
{
    ...
    if (cursor_a[0] < child.size())
    {
        ...
        for (ulong i = 1; i < cursor_a.size(); i++)
        {
...
void rootbox::draw_pre(boxdrawarg da)
{
    if (cursor_b.empty())
    {
        double y = child[cursor_a[0]].offy;
        double x = child[cursor_a[0]].offx;
        boxbase* u = child[cursor_a[0]].cbox;
        for (ulong i = 1; i < cursor_a.size(); i++)
        {
            y += offyat(u, cursor_a[i]);
    ...
    y += da.globy;
    drawtline(0, y, glb_image.sizex, y, da.nb->magnification, da.nb->cCursor, da.T);
}
...
void rootbox::draw_post(boxdrawarg da)
{
    ...
    else
    {
        ...
        for (ulong i = 0; i < cursor_a.size(); i++)
        {
            if (i >= cursor_b.size()) // should not happen
                break;
