...
void underoverscriptbox::print(size_t indent, double offx, double offy)
{
    for (size_t i = 0; i < indent; i++)
        printf("    ");
    printf("(%f,%f) underoverscript: size (%f,%f:%f)\n", offx, offy, sizex, sizey, centery);
        ...
        case movearg_Up:
        {
            if (cursor == 0)
            {
                ...
                if (r == moveret_OK)
                    return r;
                cursor = 2;
                over.cbox->move(b, movearg_First);
                return moveret_OK;
            }
            else if (cursor == 2)
            {
            ...
            else
            {
                ...
                if (r == moveret_OK)
                    return r;
                cursor = 0;
                body.cbox->move(b, movearg_First);
                return moveret_OK;
            }
        }
        case movearg_Down:
        {
            if (cursor == 0)
            {
                ...
                if (r == moveret_OK)
                    return r;
                cursor = 1;
                under.cbox->move(b, movearg_First);
                return moveret_OK;
            }
            else if (cursor == 1)
            {
...
void underoverscriptbox::measure(boxmeasurearg ma)
{
    ...
    ma.level++;
    under.cbox->measure(ma);
    over.cbox->measure(ma);
    ...
    over.offx = 0.5*(sizex - over.cbox->sizex);
    body.offx = 0.5*(sizex - body.cbox->sizex);
    under.offx = 0.5*(sizex - under.cbox->sizex);
    over.offy = 0;
